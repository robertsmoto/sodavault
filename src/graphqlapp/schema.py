...
class BundleOptionNode(DjangoObjectType):
    class Meta:
        model = Bundle
        fields = ["product", "subproduct", "quantity_min", "quantity_max", 
            "is_optional"]
        filter_fields = []
        interfaces = (relay.Node, )   
...
class PromotionNode(DjangoObjectType):
    class Meta:
        model = Promotion
        fields = ["product", "promotion_override", "name", "slug", "begins",
...
class MarketingNode(DjangoObjectType):
    class Meta:
        model = Marketing
        fields = ["product", "description_sm", "description_md", "description_lg",
            "img_1x1_lg", "img_1x1_md", "img_1x1_sm", "img_2x1_lg", "img_2x1_md",
            "img_2x1_sm", "img_1x2_lg", "img_1x2_md", "img_1x2_sm", "img_16x9", 
            "img_191x1"]
        filter_fields = []
        interfaces = (relay.Node, )   
...
class ProductNode(DjangoObjectType):
    class Meta:
        ...
        fields = ["name", "sku", "cost", "price", "categories", "tags", 
            "identifiers", "measurements", "digital_options",
...
class CampaignNode(DjangoObjectType):
    class Meta:
        model = Campaign
        fields = ["name", "date_added", "date_expires", "site_name", "site_url",
...
class AssettNode(DjangoObjectType):
    class Meta:
        model = Assett
        fields = ["campaign", "product", "name", "excerpt", "url_name", "url_link",
            "img_1x1"]
        filter_fields = []
        interfaces = (relay.Node, )   
...
class BannerNode(DjangoObjectType):
    class Meta:
        model = Banner
        fields = ["campaign", "image_xl", "image_lg", "image_md", 
